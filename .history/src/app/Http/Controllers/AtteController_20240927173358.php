...
class AtteController extends Controller
{
    ...
    public function workStart() {
        ...
        if(isset($old($oldTimestampDay == $newTimestampDay) && (empty($oldTimestamp->end_time))) {
    ...
    public function restEnd() {
        ...
        if(empty($rest) || !empty($recentRest->end_time)) {
            return back()->with('error', '既に休憩終了しているか、休憩開始ボタンが押されていません');
        }
        $rest->update([
            'end_time' => Carbon::now()
        ]);
        $restDiffInSeconds =  $rest->restDiffInSeconds($rest);
        $totalDiffInSeconds = $rest->totalDiffInSeconds($work, $restDiffInSeconds);

        $work->update([
            'rest_time' => $totalDiffInSeconds
        ]);

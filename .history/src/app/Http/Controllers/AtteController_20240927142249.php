...
class AtteController extends Controller
{
    ...
    public function attendance() {
        ...
        return view('attendance', compact('workRecords'));
    }
    ...
    public function workEnd() {
        ...
        //勤務時間の計算
        $work_time = $work->WorkDiffInSeconds() - $work->rest_time;
    ...
    public function restEnd() {
        $work = Work::where('user_id', Auth::id())->latest()->first();
        $recentRest = Rest::with('work')->where('work_id', $work->id)->latest()->first();
        if(empty($recentRest) || !empty($recentRest->end_time)) {
        ...
        $rests = Rest::with('work')->where('work_id', $work->id)->get();
        $totalRest = 0;
        foreach($rests as $rest) {
            $restDiffInSeconds = $rest->restDiffInSeconds();
            $totalRest += $restDiffInSeconds;
        }
